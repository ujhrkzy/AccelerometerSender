...
public class OrientationEstimater {
    ...
    private final float[] rotationMatrix_t1 = new float[16];
    private float[] rotationMatrix_t2 = new float[16];
    ...
    private float[] getRotationMatrix() {
        ...
        Matrix.multiplyMM(outputRotationMatrix, 0, rotationMatrix_t1, 0,
                rotationMatrix_d, 0);
        // Matrix.translateM(outputRotationMatrix, 0, 0f, posVec.values[1] *
        // -0.01f, 0f);
    ...
    private void rotateInDisplay(float dx, float dy) {
        ...
        if (l > 0.001f) {
            ...
            Matrix.multiplyMM(rotationMatrix_t2, 0, rotationMatrix_t1, 0,
                    outputRotationMatrix, 0);
    ...
    private void translateInDisplay(float[] pos, float dx, float dy, float dz) {
        float scale = 0.1f;
    ...
    public void onSensorEvent(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            if (lastAccelTime > 0 && dt < 0.5f
                    && System.currentTimeMillis() - resetTime > 500) {
                ...
                accHistory[(accHistoryCount++) % accHistory.length] = accVec
                        .length();
                ...
                posIntegretedError += vVec.length() * 0.0001f + accVec.length()
                        * 0.1f;
                ...
                eventCount++;
                if (eventCount % 20 == 0) {
                    Log.d("OrientationEstimater", "" + event.timestamp + ", "
                            + posVec + ", " + vVec + ", " + accVec + ", AL:"
                            + accVec.length() + (resting ? " R" : ""));
                }

                // Log.d("Sensor", "TYPE_PRESSURE pressureHeightCurrent: " +
                // pressureHeightCurrent + ", " + posVec.values[1]);
                if (pressureHeightErrorBaseTime > 0 && applyPressureHeight) {
                    float eh = pressureHeightErrorHigh
                            + pressureHeightErrorFactor
                            * ((event.timestamp - pressureHeightErrorBaseTime) * 0.000001f);
                    float el = pressureHeightErrorLow
                            + pressureHeightErrorFactor
                            * ((event.timestamp - pressureHeightErrorBaseTime) * 0.000001f);
            ...
            if (pressHistoryCount >= pressHistory.length) {
                ...
                if (max - min > 0.025f) {
                    Log.d("Sensor", "TYPE_PRESSURE change " + v + "min:max = "
                            + min + ":" + max);
                }
                ...
                pressureHeightCurrent = (pressureHeightBase - v) * 8300f; // todo:
                                                                          // h=153.8*(temp+273.2)*(1-(v/ATOM_BASE)^0.1902)
                // float eh = pressureHeightErrorHigh +
                // pressureHeightErrorFactor * ((event.timestamp -
                // pressureHeightErrorBaseTime) * 0.000001f);
                // float el = pressureHeightErrorLow + pressureHeightErrorFactor
                // * ((event.timestamp - pressureHeightErrorBaseTime) *
                // 0.000001f);
                // Log.d("Sensor","TYPE_PRESSURE pressureHeightCurrent " +
                // pressureHeightCurrent );
            }
            ...
            if (lastGyroTime > 0) {
                ...
                Matrix.rotateM(rotationMatrix, 0, gyroVec.length() * dt * 180
                        ...
                        gyroVec.array()[2]);
                posIntegretedError += gyroVec.length() * dt * 5.0f; // TODO:
                                                                    // error
                                                                    // ratio
                                                                    // control.
            }
        ...
        if (gyroVec.length() < 0.3f && Math.abs(accVecN.length() - G) < 0.5f) {
            ...
            Matrix.multiplyMV(tmpVec.array(), 0, rotationMatrix, 0,
                    accVec.values, 0);
            float theta = (float) Math.acos(tmpVec.dot(gravityVecI));
            if (theta > 0) {
                float[] cross = tmpVec.cross(gravityVecI).normalize().array();
                float factor = (System.currentTimeMillis() - resetTime < 500) ? 0.9f
                        : 0.0005f;

                // Matrix.rotateM(rotationMatrix, 0, theta * 180 / PI * factor,
                // cross[0], cross[1], cross[2]);
                Matrix.setRotateM(rotationMatrix_t1, 0, theta * 180 / PI
                        * factor, cross[0], cross[1], cross[2]);
                Matrix.multiplyMM(rotationMatrix_t2, 0, rotationMatrix_t1, 0,
                        rotationMatrix, 0);
                float tm[] = rotationMatrix_t2;
